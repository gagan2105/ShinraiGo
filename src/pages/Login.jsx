...
import { signInWithEmailAndPassword, signInWithPopup, GoogleAuthProvider, getRedirectResult, browserLocalPersistence, setPersistence } from "firebase/auth";
import { auth } from "../lib/firebase";
import { useNavigate, Link } from "react-router-dom";
import axios from "axios";
import { Shield, Mail, Lock, LogIn, Loader2, CheckCircle2, Globe, Zap, Bell, ShieldCheck, Fingerprint } from "lucide-react";
...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            try {
                ...
                setSuccessAnim(true);
                toast.success("Welcome back to Shinrai!");
    ...
    const handleDummyLogin = async (role) => {
        setLoading(true);
        try {
            // Pre-sync with backend using dummy token to ensure user doc exists
            const dummyToken = `DUMMY_${role.toUpperCase()}_TOKEN`;
            await axios.post(ENDPOINTS.SYNC, { name: "Prototype User" }, {
            ...
            <div className="hidden md:flex md:w-[45%] lg:w-[40%] bg-slate-900 relative p-16 flex-col justify-between border-r border-slate-800">
                ...
                <div className="relative z-10">
                    ...
                    <h1 className="text-5xl font-black text-white mb-8 leading-[1.1] tracking-tight">
                        Security at <span className="text-blue-500">Scale.</span>
                    </h1>
                    ...
                    <div className="space-y-8 mt-12">
                        <FeaturePoint 
                            ...
                            desc="Secondary authentication layer for high-risk regions."
                        />
                        <FeaturePoint 
                            icon={<Zap className="w-5 h-5 text-amber-500" />}
                            title="Zero Latency"
                            desc="Real-time data synchronization with central police command."
                        />
                    </div>
                </div>

                <div className="relative z-10 pt-10 border-t border-slate-800">
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-[0.3em] mb-4">Neural Protocol v2.4</p>
                    <div className="flex items-center space-x-2 text-emerald-500">
                        <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                        <span className="text-[10px] font-black uppercase tracking-widest">Command Center Online</span>
                    </div>
                </div>
            </div>
                ...
                <div className="max-w-md w-full">
                    ...
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <button
                            ...
                            className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-2xl flex items-center justify-center border-none transition-all shadow-xl shadow-blue-500/20 disabled:opacity-50 mt-6 group overflow-hidden relative"
                        >
                    ...
                    <p className="mt-10 text-center text-slate-500 text-sm">
                        New to the agency?{" "}
                        <Link to="/signup" className="text-blue-500 font-bold hover:underline">Request Enrollment</Link>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-2">
            ...
            <div className="relative group/input">
                ...
                <input
                    ...
                    value={value}
                    onChange={onChange}
                    className="w-full bg-slate-900/50 border border-slate-800 rounded-2xl pl-12 pr-4 py-4 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all hover:bg-slate-900"
